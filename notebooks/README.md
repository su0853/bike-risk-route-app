...
- **purpose**：看路網高程 `z` / 坡度 `grade_abs` 分佈、哪類路最陡，並用北投→淡水的**高程剖面**示範
  `λ_slope` 的避坡效果（對應 006 §1 坡度權重）。
- **needs**：先跑過 `download_dem` + `rebuild_from_db`（graph pkl 帶 `z`）；風險分數走 DB（postgis 起著）。
- **scale**：L3。
- **runtime**：< 1 分鐘（載 graph pkl ~10s）。
- **output**：保留（含圖）。
- 結果：z 中位 `36 m`、P99 `1269 m`；長度加權平均坡度 `4.5%`，最陡類別 `path 12.8%`；
