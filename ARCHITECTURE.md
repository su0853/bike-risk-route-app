...
本系統為台灣自行車安全導航應用（路網來源為 Geofabrik Taiwan，覆蓋台灣全島），核心功能為：
...
使用者輸入起終點
      │
      ▼
  前端輸入地址 → GET /api/geocode（後端 proxy）→ Nominatim
  多候選選擇 UI → 使用者確認地點 → lat/lon
      ▼
POST /api/navigate
      │
      ├─── asyncio.gather ─────────────────────────────────┐
...
嚴重程度分數（依事故類型分別設定）：
  if case_type == 'A1':
      severity = death_count × 3.0 + injury_count × 1.5
      # A1：24小時內死亡；A1 中的受傷者也在較嚴重情境下
  if case_type == 'A2':
      severity = death_count × 2.0 + injury_count × 1.0
      # A2：受傷或 24小時後死亡；A2 死亡次嚴重
...
├── components/
│   ├── SearchForm.tsx        起終點輸入（多候選選擇 UI + 目前位置按鈕）+ λ 滑桿 + 送出按鈕
...
│   └── geocoder.ts          後端 proxy 客戶端；geocodeAddressCandidates()（多候選）/ geocodeAddress()（取第一筆）
└── types/
    └── navigation.ts         RouteResult、NavigateRequest/Response、GeocoderResult 型別
```

#### Geocoding 流程

前端不直接呼叫 Nominatim。地址查詢透過後端 proxy：

```
前端輸入文字 → GET /api/geocode?q=... → backend/app/routers/geocode.py → Nominatim
                                         ↓
                              回傳最多 5 筆候選結果 [{lat, lon, display_name}]
                                         ↓
                              SearchForm 顯示候選清單 → 使用者點選確認
```

Android 直接呼叫 Nominatim 會收到 HTTP 403（Nominatim 阻擋 Android fetch 的 User-Agent），後端 proxy 解決此問題。

#### SearchForm 欄位狀態

每個欄位（起點 / 終點）使用 `FieldState` 管理四個狀態：

```typescript
interface FieldState {
  text: string;         // 輸入框文字
  candidates: GeocoderResult[];  // 候選清單
  selected: GeocoderResult | null;  // 已確認選定
  searching: boolean;   // 查詢進行中
}
```

起點欄位另有「定位」按鈕，呼叫 `expo-location` 取得 GPS 座標作為起點（不做 reverse geocoding，直接填入座標）。

兩端都 `selected !== null` 才啟用「搜尋路線」按鈕。
...
| 參數 | 預設值 | 說明 |
|------|--------|------|
| `RISK_A1_DEATH_WEIGHT` | `3.0` | A1 死亡（24h 內）係數（待 VSL 研究更新） |
| `RISK_A1_INJURY_WEIGHT` | `1.5` | A1 事故中受傷者係數 |
| `RISK_A2_DEATH_WEIGHT` | `2.0` | A2 死亡（24h 後）係數 |
