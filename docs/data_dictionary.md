...
事故資料的本質決定它能回答哪些問題、不能回答哪些問題。

**資料特性**

- **點型**：每筆事故是一個座標點（政府原始經緯度）。
- **計數性**：聚合到分析單位（路段、網格）後，單位內事故數是 `0, 1, 2, …` 的計數，非連續常態資料。
- **時間性**：事故時間可衍生年、月、星期與時段，用於描述時間分布並探索可能的趨勢與季節性；目前資料僅涵蓋 2018–2020，不足以單獨確認長期穩定趨勢。
- **暴露量依賴**：事故數同時反映「環境危險」與「通行量多寡」，可概念化為「觀測事故數 ≈ 通行暴露量 × 單位暴露事故率」——事故多的路段可能只是騎的人多。

**可用於**

- 描述已觀測事故位置之間的空間聚集與局部高低組合；低值格不直接解讀為安全區。
- 描述事故的時間分布，並探索可能的趨勢與季節性。
- 作為現行風險分數的來源（嚴重度 × 時間衰減的事故密度）。

**限制（資料層）**

- **原始事故點資料只有事故紀錄（case-only）**：資料沒有直接記錄「有通行但未發生事故」的路段／時段，也沒有通行量（暴露量）分母，因此只能得到**事故密度**，得不到「每次通行的風險率」。下游會將未匹配到事故的道路設為 `risk=0`，但這些零不能直接解讀為已觀測到的安全負樣本。
- **零值有兩種、且資料本身分不出**：結構零（根本沒人通行，如死巷、禁行路段）與隨機零（有通行但恰好未發生）；要區分需道路可通行性、連通性、暴露量等外部資訊。
- **通報偏差**：輕微事故可能漏報；各年度通報制度可能不同。
- **定位誤差**：原始經緯度與 snap 到道路的過程，可能把事故落到錯誤路段。
- **空間聚合限制**：將事故聚合到 H3 或路段後，結果會受到網格解析度、邊界與道路切割方式影響。
- **涵蓋期間有限**：目前專案附帶資料為 2018–2020 年（民國 107–109，2018-01-01 至 2020-12-31），不能代表最新事故狀況，也不足以單獨確認長期趨勢；2020 年可能受疫情、交通行為與通報環境改變影響。
- **資料時間對齊**：事故、道路與未來暴露量若來自不同年份，可能把道路改建後的狀態與舊事故錯誤配對。
- **外部事件影響**：事故件數會受疫情、政策及交通行為改變影響。若缺少同期自行車暴露量，只能描述事故數的時間變化，不能判斷變化來自通行機會或單位通行風險；暴露量可縮小解釋範圍，但不能控制騎乘者組成、路線、天氣與通報制度等所有外部因素。

**模型層限制與未來方向**：風險分數作為 heuristic 的假設與待校準定位，見 [risk_score_methodology.md](risk_score_methodology.md)「十一、已知限制與未來改善方向」。要從「事故密度」升級為「每次通行的風險率」，前提是取得自行車暴露量資料（目前尚無）；且暴露量需與事故在年份與時間粒度上相近，全年交通量不適合直接解釋月或時段層級的事故率。
